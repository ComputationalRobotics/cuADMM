function problem2txt(problem, output_dir)
    store_sparse_mat(problem.C, fullfile(output_dir, 'C.txt'));
    store_sparse_mat(problem.b, fullfile(output_dir, 'b.txt'));
    store_sparse_mat(problem.At, fullfile(output_dir, 'At.txt'));
    store_sparse_vec([(-1 + sqrt(1+8*size(problem.At, 1))) / 2], fullfile(output_dir, 'blk.txt'));
    store_sparse_vec(size(problem.At, 2), fullfile(output_dir, 'con_num.txt'));
end
