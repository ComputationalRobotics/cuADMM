    ...
    int vec_len)
{
    ...
    if (idx < vec_len)
    {
        ...
        m2 = map_M2_vals[idx];
        if_diag = (m1 == m2);
        if (b == MatrixSizeCategory::LARGE)
        { // to large matrix
            // if the coefficient is not on the diagonal, we need to multiply by 1/sqrt(2)
            large_mat_vals[m1] = Xb_vals[idx] * (SQRT2INV + int(if_diag) * (1 - SQRT2INV));
            large_mat_vals[m2] = large_mat_vals[m1];
        }
        else if (b == MatrixSizeCategory::MEDIUM)
        { // to medium matrix
            medium_mat_vals[m1] = Xb_vals[idx] * (SQRT2INV + int(if_diag) * (1 - SQRT2INV));
            medium_mat_vals[m2] = medium_mat_vals[m1];
        }
        else if (b == MatrixSizeCategory::SMALL)
        { // to small matrix
...
// - map_B is used to determine which coefficients of the vector correspond to which elements of the matrices.
...
// - map_B is used to determine which matrix to use for a coefficient of the vector.
